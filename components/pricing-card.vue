...
<template>
  <div class="w-full bg-white shadow-sm p-2 my-2">
    <div class="w-full relative items-start">
        <h3 class="text-lg text-primary text-center">{{ data.name }}</h3>
      <div>
    ...
    <div class="w-full mt-5">
      <table v-if="data.pricings.length">
        ...
        <tbody>
          <tr v-for="(pricing, index) in data.pricings" :key="pricing.id">
            <td>
              <span v-if="!pricing.isEditing">{{ pricing.lowerDistance }}</span>
              <input v-model.number="pricing.lowerDistance"  placeholder="Lower Distance" v-else />
            </td>
            <td>
              <span v-if="!pricing.isEditing">{{ pricing.upperDistance }}</span>
              <input v-model.number="pricing.upperDistance" placeholder="Upper distance" v-else />
            </td>
            <td>
              <span v-if="!pricing.isEditing">{{ pricing.unitPrice }}</span>
              <input v-model.number="pricing.unitPrice" placeholder="Unit price" v-else />
            </td>
            <td>
              <button
              ...
              <button v-else class="mx-2 text-green-700" @click="onSave(pricing)">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  class="h-6 w-6 fill-current"
                  viewBox="0 0 512 512"
              ...
              <button class="mx-2 text-red-500" v-else @click="onDelete(pricing.id)">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  class="h-6 w-6 fill-current"
                  viewBox="0 0 512 512"
