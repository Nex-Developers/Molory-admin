        ...
        <div class="w-full space-y-4 md:w-9/12 mx-2 h-64">
          ...
          <div class="grid grid-cols-4 gap-4">
            <div class="bg-white p-3 shadow-sm rounded-sm">
              ...
              <span v-else-if="trip.status === 1" class="text-success"
                >Arrivé</span
              >
              <span v-else-if="trip.status === 2" class="text-primary"
                >En cours</span
              >
              <span v-else-if="trip.status === 3" class="text-secondary"
                >En attente</span
              >
            </div>
          ...
          <div class="bg-white p-3 shadow-sm rounded-sm">
            ...
            <div
              class="grid grid-cols-3 items-center border border-tertiary text-xs rounded-md p-2"
            >
                ...
                <div>
                  ...
                  >
                    ...
                    <div class="w-1/2">
                      ...
                      <div
                        class="capitalize text-gray-600 text-sm pt-2 text-center"
                      >
                    ...
                    <div class="w-1/4">
                      <span class="text-gray-600 text-sm"
                        >{{ item.travel.seats }} seats</span
                      >
                    </div>
                    <div class="w-1/4">
                      <div class="">
                        <span
                          class="text-white py-1 px-2 w-24 text-center block rounded-l-full rounded-r-full bg-blue-900"
                          v-if="item.travel?.status === 5 || item.travel?.status === 4"
                          >En attente</span
                        >
                        <span
                          class="text-white py-1 px-2 w-24 text-center block rounded-l-full rounded-r-full bg-purple-900"
                          v-else-if="
                          item.travel?.status === 3 || item.travel?.status === 2
                          "
                          >En route</span
                        >
                        <span
                          class="text-white py-1 px-2 w-24 text-center block rounded-l-full rounded-r-full bg-green-900"
                          v-if="item.travel?.status === 1"
                          >Arrivé</span
                        >
                        <span
                          class="text-white py-1 px-2 w-24 text-center block rounded-l-full rounded-r-full bg-red-700"
                          v-if="item.travel?.status === 0"
                          >Annulé</span
                        >
                      </div>
                    </div>
...
const amount = trip.passengers.reduce(
  (a: any, b: any) => a + b.travel.route.price,
  0
);
</script>